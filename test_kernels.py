...
log_file_path = "kernel_sweep_results.txt"
...
def extract_robust_contour(crop_f_resized, mask_circle, kernel_size, prev_contour=None, prev_area=None):
    ...
    for th in thresholds:
        ...
        for c in contours_seq:
            ...
            if prev_centroid is not None and prev_area is not None:
                ...
                # Soft recovery logic
                is_recovery = (prev_area < 1000) and (c_area > 2200) and (solidity > 0.85) and (dist_from_center < 35)
...
def evaluate_with_kernel(video_path, kernel_size):
    ...
    for i in range(total_frames):
        ...
        pts, area, _ = extract_robust_contour(crop_f_resized, mask_circle, kernel_size, prev_c, prev_a)
        prev_c = pts
        prev_a = area
    ...
    for i in range(start_f, end_f):
        ...
        crop_f = frame[y:y+h, x:x+w]
        crop_f_resized = cv2.resize(crop_f, (224, 224))
        pts, area, raw_c = extract_robust_contour(crop_f_resized, mask_circle, kernel_size, prev_c, prev_a)
        prev_c = pts
        prev_a = area
    ...
    n_collapsed = max(1, int(len(sorted_areas) * 0.3))
...
def evaluate_all():
    ...
    for k_size in kernels:
        log_print(f"\n--- Evaluating morphological closing kernel size: {k_size}x{k_size} ---")
        agreed_matches = 0
        overall_matches = 0
        ...
        for i in range(1, 31):
            ...
            res = evaluate_with_kernel(filepath, k_size)
            ...
            if match:
                overall_matches += 1
                if i in [4, 6, 13, 14, 20, 21, 24]:
                    agreed_matches += 1
            
            status = "MATCH" if match else "MISMATCH"
            log_print(f"Pt{i:02d} | GT: {gt['class']} Deg {gt['degree']} | Pred: {pred_class} Deg {pred_deg} ({pred_red:.1f}%) | {status}")
                    
        log_print(f"Kernel {k_size}x{k_size} Summary:")
        log_print(f"Agreement Cases: {agreed_matches}/7 ({agreed_matches/7*100:.1f}%)")
        log_print(f"Overall Cases: {overall_matches}/30 ({overall_matches/30*100:.1f}%)")
