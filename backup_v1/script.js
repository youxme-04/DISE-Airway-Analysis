...
function init3DScene() {
    ...
    camera.position.set(10, 8, 14); // Endoscopic view angle looking down the tube
...
function render3DAirway(contourSlices, sliceAreas) {
    ...
    for (let i = 0; i < slices_count; i++) {
        const yHeight = (i - Math.floor(slices_count / 2)) * ySpacing_render;
        for (let j = 0; j < M; j++) {
            const pt = referenceSlice[j];
            const x = (pt[0] - maxCx) * scale;
            const z = (pt[1] - maxCz) * scale;
            refVertices.push(x, yHeight, z);
        }
    }
    ...
    for (let i = 0; i < slices_count; i++) {
        const yHeight = (i - Math.floor(slices_count / 2)) * ySpacing_render;
        for (let j = 0; j < M; j++) {
            vertices.push(0, yHeight, 0); // Placeholder positions, will be updated in updateHighlightSlice
...
function updateHighlightSlice(idx) {
    ...
    if (airwayMesh && airwayMesh.geometry) {
        ...
        for (let i = 0; i < slices_count; i++) {
            ...
            for (let j = 0; j < M; j++) {
                ...
                posAttr.setXYZ(vertexIndex, x, yHeight, z);
    ...
    // Create glowing circle outline staying at y = 0.05
    const linePoints = [];
    const yHeightLine = 0.05;
    for (let j = 0; j < M; j++) {
        const pt = slice[j];
        const x = (pt[0] - cx) * scale;
        const z = (pt[1] - cz) * scale;
        linePoints.push(new THREE.Vector3(x, yHeightLine, z));
    }
    ...
    // Add 4 collapse direction force vectors (Top, Bottom, Left, Right) at y = 0.0
    ...
    targetIndices.forEach(j => {
        ...
        const zMax = (ptMax[1] - maxCz) * scale;
        const xCurr = (ptCurr[0] - cx) * scale;
        const zCurr = (ptCurr[1] - cz) * scale;

        const startPt = new THREE.Vector3(xMax, 0.0, zMax);
        const endPt = new THREE.Vector3(xCurr, 0.0, zCurr);
...
function showResults() {
    ...
    if (contourSlicesData.length > 0) {
        ...
        // Reset camera focus target to origin (Endoscopic view angle looking down the tube)
    ...
    document.getElementById('geom-collapse-type').textContent = backendResultData.prediction_class;
    document.getElementById('geom-confidence-value').textContent = confVal.toFixed(1) + '%';
    ...
    document.getElementById('print-geom-type').textContent = backendResultData.prediction_class;
